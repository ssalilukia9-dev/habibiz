      ...
      >
        <div className="p-8 md:p-12 space-y-8">
          ...
          <div className="text-center space-y-3">
             <div className="w-16 h-16 bg-brand-primary/10 rounded-2xl flex items-center justify-center text-brand-primary mx-auto mb-6 shadow-xl shadow-brand-primary/10 animate-pulse">
                <LogIn size={32} />
             </div>
          ...
          {/* Tab Selector between Capacitor Cloud Sync & Firebase Auth */}
          <div className="flex bg-brand-depth/40 p-1.5 rounded-2xl border border-white/5">
            <button
              onClick={() => {
                setUseRestDb(true);
                setError(null);
              }}
              className={`flex-1 py-2.5 px-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                useRestDb ? 'bg-brand-primary text-brand-depth shadow-lg shadow-brand-primary/10' : 'text-slate-400 hover:text-white'
              }`}
            >
              <RefreshCw size={12} className={useRestDb ? 'animate-spin' : ''} />
              Android Sync
            </button>
            <button
              onClick={() => {
          ...
          {useRestDb ? (
            /* Custom REST Synchronization Auth Form (Capacitor & APK Ready!) */
            <form onSubmit={handleRestAuthSubmit} className="space-y-6">
              <div className="space-y-2">
                <h3 className="text-xs font-black text-brand-primary uppercase tracking-wider">
                  {isRegister ? 'Create Cloud Sync Account' : 'Sign In to Cloud Sync'}
                </h3>
                <p className="text-xs text-slate-400 font-medium leading-relaxed">
                  {isRegister 
                    ? 'Establish a secure credentials-based account to back up streaks, Hasanat points, books, and enable feed posts and chat rooms.'
                    : 'Log back in with your password to restore and sync your full spiritual journey instantly.'}
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  className="text-xs text-slate-400 hover:text-white font-bold py-2 transition-colors"
                >
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <h3 className="text-xs font-black text-brand-primary uppercase tracking-wider">Secure Firebase Account</h3>
                  <p className="text-xs text-slate-400 font-medium leading-relaxed">
              ...
              {/* Direct Password-free local registration */}
