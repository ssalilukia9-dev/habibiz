...
export default function ProfileView({ 
  ...
  useEffect(() => {
    ...
    if (currentUser.uid.startsWith('local_') || currentUser.uid.startsWith('rest_')) {
      ...
      if (restUser && currentUser.uid.startsWith('rest_')) {
        const data = {
          displayName: restUser.displayName,
          photoURL: `https://api.dicebear.com/7.x/avataaars/svg?seed=${restUser.uid}`,
          bio: restUser.bio || 'Wandering the offline path...',
          hasanat: restUser.hasanat || 0,
          streak: restUser.streak || 0,
          createdAt: restUser.createdAt || new Date().toISOString(),
        ...
        if (existingData) {
          ...
          const defaultData = {
            displayName: currentUser.displayName || 'Spirit Seeker',
            photoURL: currentUser.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${currentUser.uid}`,
            bio: 'Wandering the offline path...',
            hasanat: Number(localStorage.getItem(`hasanat_local_${currentUser.uid}`)) || 0,
            streak: Number(localStorage.getItem(`hadith_streak_${currentUser.uid}`)) || 0,
            createdAt: new Date().toISOString(),
            lastSeen: new Date().toISOString()
          };
          setUserData(defaultData);
          setEditName(defaultData.displayName);
          setEditPhoto(defaultData.photoURL);
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentUser.uid.startsWith('local_') || currentUser.uid.startsWith('rest_')) {
        if (currentUser.uid.startsWith('rest_')) {
          const restUser = restDbClient.getUser();
          const success = await restDbClient.sync(
            userData.hasanat || 0,
            userData.streak || 0,
            userData.bookmarks || [],
            editBio,
          ...
          if (success) {
            const updated = await restDbClient.getProfile();
            setUserData({
              ...userData,
              displayName: updated.displayName,
              bio: updated.bio || 'Wandering the offline path...'
            });
            localStorage.setItem('temp_onboarding_name', editName);
            notificationService.notify('Profile Synchronized', 'Your cloud identity records have been updated.', 'system');
            setIsEditing(false);
            return;
          }
        } else {
          ...
          notificationService.notify('Profile Synchronized', 'Your guest identity records have been updated.', 'system');
          setIsEditing(false);
          // Dispatch an event to notify App.tsx that profile changed so it updates any cached guest names
          window.dispatchEvent(new CustomEvent('guest_profile_updated'));
          return;
        }
      }
  ...
  const lastSeenDate = userData?.lastSeen 
    ? (userData.lastSeen.toDate ? userData.lastSeen.toDate().toLocaleString() : new Date(userData.lastSeen).toLocaleString()) 
    : 'N/A';
