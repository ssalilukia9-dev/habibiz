...
const COLLECTIONS = [
  ...
  { id: 'Sahih Muslim', name: 'Sahih Muslim', color: 'bg-green-500' },
  { id: 'Tirmidhi', name: 'Sunan al-Tirmidhi', color: 'bg-amber-500' },
      ...
      <AnimatePresence mode="wait">
        {!selectedHadith ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredHadith.map((h, i) => (
                <motion.button
                  ...
                  className="glass-panel p-6 rounded-3xl text-left border-white/5 hover:border-brand-primary/20 transition-all group overflow-hidden relative"
                >
                  ...
                  <p className="text-slate-300 text-sm leading-relaxed line-clamp-3 mb-4 font-light italic">
                    "{h.english}"
                  </p>
                  <div className="flex items-center justify-between mt-auto">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Reported by {h.narrator}</span>
          ...
          >
            ...
            <div className="glass-panel p-10 md:p-16 rounded-[3rem] border-brand-primary/20 relative overflow-hidden shadow-2xl">
              ...
              <div className="relative z-10 space-y-12">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Collection</p>
                      <p className="text-sm font-bold text-slate-200">{selectedHadith.collection}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                     <button className="p-3 glass-panel rounded-xl text-slate-400 hover:text-brand-primary transition-all">
                        <Bookmark size={20} />
                     </button>
                     <button className="p-3 glass-panel rounded-xl text-slate-400 hover:text-brand-primary transition-all">
                        <Share2 size={20} />
                ...
                <div className="space-y-10 text-center">
                  <p className="arabic-text text-4xl md:text-5xl text-brand-primary leading-[1.8] text-right" dir="rtl">
                    {selectedHadith.arabic}
                  </p>
                  <div className="w-20 h-1 bg-brand-primary/20 mx-auto rounded-full" />
                  <p className="text-slate-300 italic text-2xl font-light leading-relaxed">
                    "{selectedHadith.english}"
                  </p>
                </div>

                <div className="flex flex-col md:flex-row justify-center items-center gap-6 pt-10 border-t border-white/10">
                   <div className="text-center md:text-left">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Topic</p>
                      <p className="text-sm font-black text-brand-primary uppercase tracking-wider">{selectedHadith.topic}</p>
                   </div>
                   <div className="w-px h-10 bg-white/10 hidden md:block" />
                   <div className="text-center md:text-left">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Narrated by</p>
                      <p className="text-sm font-black text-slate-200">{selectedHadith.narrator}</p>
                   </div>
                </div>
              </div>
            </div>
