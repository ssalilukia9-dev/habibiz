...
const RECITERS = [
  ...
  { id: 3, name: 'Abdur-Rahman as-Sudais', slug: 'ar.as-sudais', sub: 'Imam of Grand Mosque' },
  { id: 6, name: 'Maher Al-Muaiqly', slug: 'ar.maheralmuaiqly', sub: 'Distinctive clear tone' },
...
export default function SurahDetail({ 
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const localData = (quranSample as any)[surah.number.toString()];
      if (localData) {
        setAyahs(localData.ayahs);
        setIsLoading(false);
        return;
      }

      try {
        const resArabic = await fetch(`https://api.alquran.cloud/v1/surah/${surah.number}`);
  ...
  const playAyah = (ayah: any) => {
    ...
    const audioUrl = `https://cdn.islamic.network/quran/audio/128/${reciter?.slug}/${ayah.number}.mp3`;
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-brand-primary gap-6">
        <Loader2 className="animate-spin" size={48} />
        <p className="font-bold tracking-[0.2em] uppercase text-xs">Opening Sanctuary...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-12 pb-32">
      {/* Surah Header */}
      <div className="bg-brand-sidebar/95 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] border border-brand-border flex items-center justify-between sticky top-4 z-40 shadow-2xl shadow-black/40">
        <div className="flex items-center gap-6">
          <button onClick={onBack} className="w-12 h-12 bg-white/5 border border-white/5 rounded-2xl flex items-center justify-center text-brand-primary hover:bg-brand-primary/20 transition-all shadow-inner">
            <ChevronLeft size={24} />
          </button>
          <div>
            <h2 className="text-2xl font-bold text-white tracking-tight">{surah.englishName}</h2>
            <div className="flex items-center gap-2">
               <span className="text-[10px] text-brand-primary font-bold uppercase tracking-[0.2em]">{surah.revelationType} • {surah.numberOfAyahs} Ayahs</span>
               <div className="w-1 h-1 bg-slate-700 rounded-full" />
               <button 
                 onClick={() => setShowReciters(!showReciters)}
                 className="text-[10px] text-slate-400 hover:text-brand-primary font-bold uppercase tracking-widest flex items-center gap-1 transition-colors"
               >
                 <Volume2 size={12} /> {RECITERS.find(r => r.id === selectedReciter)?.name.split(' ').pop()}
               </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="text-right hidden sm:block">
            <p className="arabic-text text-4xl font-bold text-brand-primary leading-tight drop-shadow-[0_0_15px_rgba(212,175,55,0.3)]">{surah.name}</p>
          </div>
        </div>
      ...
      {surah.number !== 1 && surah.number !== 9 && (
        <div className="flex flex-col items-center py-16 gap-4">
          <div className="text-brand-primary/40 text-xl font-serif italic mb-2 tracking-widest">Bismillahir Rahmanir Raheem</div>
          <p className="arabic-text text-5xl text-brand-primary/90 tracking-wide drop-shadow-[0_0_20px_rgba(212,175,55,0.2)]">
        ...
        {ayahs.map((ayah) => (
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-8">
               <div className="w-10 h-10 border border-brand-primary/30 rounded-full flex items-center justify-center text-brand-primary text-xs font-bold font-mono">
                  {ayah.numberInSurah}
               </div>
               ...
               <div className="flex items-center gap-3">
                  ...
                  >
                    {playingAyah === ayah.number ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}
                  </button>
                  <button 
                  ...
                  >
                    <Bookmark size={18} fill={isBookmarked(ayah.number) ? "currentColor" : "none"} />
                  </button>
                  <button className="text-slate-500 hover:text-brand-primary p-2 transition-colors"><Share2 size={18} /></button>
                  <button 
            ...
            <div className="mb-10 text-center">
              <p className="arabic-text text-5xl md:text-6xl leading-[1.8] text-white drop-shadow-[0_0_15px_rgba(212,175,55,0.15)] group-hover:drop-shadow-[0_0_20px_rgba(212,175,55,0.25)] transition-all">
                {ayah.text}
              </p>
            </div>
            ...
            <div className="max-w-3xl mx-auto text-center px-4">
              <p className="text-xl md:text-2xl font-light text-slate-300 italic tracking-tight leading-relaxed">
            ...
            {showTafsir === ayah.number && (
              ...
              >
                ...
                <p className="text-slate-400 leading-relaxed text-sm md:text-base relative z-10 font-medium">
                  {/* Mock content updated for blue/gold theme */}
      ...
      {/* Play Bar */}
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-40 bg-brand-sidebar/80 backdrop-blur-2xl border border-brand-primary/20 rounded-full px-8 py-4 shadow-2xl flex items-center gap-8 min-w-[320px]">
         <button 
           onClick={() => setAutoPlay(!autoPlay)}
           className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${autoPlay ? 'text-brand-primary' : 'text-slate-500'}`}
         >
         ...
         <button 
           ...
           className="w-14 h-14 bg-brand-primary text-brand-depth rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg shadow-brand-primary/20"
         >
           {playingAyah ? <Pause fill="currentColor" size={24} /> : <Play fill="currentColor" size={24} />}
         </button>
      ...
      <AnimatePresence>
        {playingAyah && (
          <motion.div
            ...
            className="fixed bottom-24 lg:bottom-10 left-1/2 -translate-x-1/2 w-[90%] md:w-fit min-w-[300px] glass-panel-gold border-brand-primary/40 p-4 rounded-[2rem] z-50 flex items-center gap-6 shadow-2xl backdrop-blur-2xl"
          >
             <div className="w-12 h-12 bg-brand-primary rounded-2xl flex items-center justify-center text-brand-depth animate-pulse">
                <Volume2 size={24} />
             </div>
             <div className="flex-1">
                <p className="text-[8px] font-black text-brand-primary uppercase tracking-[0.2em] mb-0.5">Now Reciting</p>
