...
class OfflineService {
  ...
  async saveAyahs(surahNumber: number, ayahs: Ayah[], reciterId?: number) {
    ...
    if (user && !user.uid.startsWith('local_')) {
      try {
        const docRef = doc(db, 'users', user.uid, 'downloadedSurahs', metaKey);
        await setDoc(docRef, {
          surahNumber,
          surahName: `Surah ${surahNumber}`,
          reciterId: reciterId ? reciterId.toString() : 'default',
          syncedAt: new Date().toISOString()
        }, { merge: true });
      } catch (e) {
        console.error('Error syncing downloaded surah to Firestore:', e);
      }
    }
  }
  ...
  async removeDownloadedSurah(surahNumber: number, reciterId?: number) {
    ...
    if (user && !user.uid.startsWith('local_')) {
      try {
        ...
        console.error('Error removing downloaded surah from Firestore:', e);
      }
    }
  }
